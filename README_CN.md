...
### Linux 平台
- **技术栈**：基于 eBPF (Extended Berkeley Packet Filter) 技术，采用 **CO-RE (Compile Once - Run Everywhere)** 编写
- **数据源**：内核网络数据包捕获和解析（Socket Filter 机制，支持解析出实际的 IP 响应结果）
- **监控点**：udp_sendmsg 和 tcp_sendmsg 系统调用，结合 Kprobe / Kretprobe 精准关联 PID
- **权限要求**：需要 root 权限或特权模式
- **卓越的跨发行版与兼容性设计**：
  - **发行版免链接支持**：采用 Go 纯静态链接编译（`CGO_ENABLED=0`），不依赖 `glibc` 动态库，完美在 Alpine Linux (`musl libc`)、CentOS/RHEL、Alma/Rocky、Debian、Ubuntu 等主流发行版上无差别直接运行。
  - **运行时零环境依赖**：生成的 eBPF 字节码以 `go:embed` 内嵌在最终程序中，目标主机**无需安装** Clang、LLVM、GCC 等编译套件，直接下载单文件即可秒级部署。
  - **跨内核自适应**：基于 CO-RE 重定位技术，程序在启动时会自动读取当前主机的内核 BTF 描述信息，并动态修正 eBPF 字节码中的结构体偏移量。即使在旧主机、经过第三方深度定制或安全加固的内核上，程序依旧能实现免重新编译的完美自适应。
